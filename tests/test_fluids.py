...
@pytest.mark.parametrize('dummy_values', [
    ...
    ('pa', 't', 'G', (673174274.6197122, 1.68291455e+10)),
    ('p', 'ta', 'G', (6.73174275e+08, 7.49509086e+08)),
    ('pa', 'ta', 'G', (6.73174275e+08, 1.87375111e+10)),
], indirect=True)
...
def test_oil_isothermal_density(dummy_values, tol):
    ...
    p, rho = param
    assert np.allclose(
        bw92.oil_isothermal_density(rho, p/1E6),
        ans, rtol=tol['rel']
    )
...
def test_oil_density(dummy_values, tol):
    ...
    p, rho, t = param
    assert np.allclose(
        bw92.oil_density(rho, p/1E6, t),
        ans, rtol=tol['rel']
    )

@pytest.mark.parametrize('dummy_values', [
    ('orho', 'G', 'rg', 't', 1.57823582)
], indirect=True)
def test_oil_fvf(dummy_values, tol):
    param, ans = dummy_values
    rho, G, rg, t = param
    assert np.allclose(
        bw92.oil_fvf(rho, G, rg, t),
        ans, rtol=tol['rel']
    )

@pytest.mark.parametrize('pres, extrap, ans', [
    (325, 'const', 1.4615),
    (325, 'pchip', 1.4615),
...
def test_wat_velocity_pure(dummy_values, tol):
    ...
    t, p = param
    assert np.allclose(
        bw92.wat_velocity_pure(t, p/1E6),
        ans, rtol=tol['rel']
    )
...
def test_wat_velocity_brine(dummy_values, tol):
    ...
    t, p, sal = param
    assert np.allclose(
        bw92.wat_velocity_brine(t, p/1E6, sal/1E6),
        ans, rtol=tol['rel']
    )
...
def test_wat_density_brine(dummy_values, tol):
    ...
    t, p, sal = param
    assert np.allclose(
        bw92.wat_density_brine(t, p/1E6, sal/1E6),
        ans, rtol=tol['rel']
    )
